...
A6 = ${fparse 0.25 * pi * D5^2}
A7 = ${fparse 0.25 * pi * D5^2}
A8 = ${fparse 0.25 * pi * D5^2}
...
x1_in = ${fparse x1 + 0.001}
x1_out = ${fparse x1 + L1 - 0.001}
x2_in = ${fparse x2 + 0.001}
x2_out = ${fparse x2 + L2 - 0.001}
y3_in = ${fparse y3 - 0.001}
y3_out = ${fparse y3 - L3 + 0.001}
x4_in = ${fparse x4 - 0.001}
x4_out = ${fparse x4 - L4}
y5_in = ${fparse y5 + 0.001}
y5_out = ${fparse y5 + L5 - 0.001}
x6_in = ${fparse x6 + 0.001}
x6_out = ${fparse x6 + L6 - 0.001}
x7_in = ${fparse x7 + 0.001}
x7_out = ${fparse x7 + L7 - 0.001}
...
[Functions]
  ...
  [is_tripped_fn]
    type = ParsedFunction
    vars = 'motor_torque turbine_torque'
    vals = 'motor_torque turbine_torque'
  ...
  [set_PID_tripped_fn]
    ...
    value = 1
  []
  [get_PID_trip_status_fn]
    type = ParsedFunction
    vals = 'PID_trip_status'
    vars = 'PID_trip_status'
  ...
  [motor_torque_fn_shutdown]
    type = ParsedFunction
    vals = 'PID_trip_status trip_time'
    vars = 'PID_trip_status trip_time'
    value = 'if(PID_trip_status = 1, max(3.5 - (3.5 * ((t - trip_time) / 25000)),0.0), 1)'
  []

  #Generatres motor power curve
  [motor_power_fn]
    type = ParsedFunction
    value = 'torque * speed'
    vars = 'torque speed'
  ...
  []
  #Generatres generator power curve
  [generator_power_fn]
    type = ParsedFunction
    value = 'torque * speed'
    vars = 'torque speed'
...
[Components]
  ...
  [recuperator]
    ...
    materials = steel
    inner_radius = ${D1}
    outer_radius = ${fparse D1 + recuperator_width}
  []
...
[ControlLogic]
  ...
  [set_point]
    ...
    function = ${fparse speed_rated_rpm - 2400}
  []
  ...
  [logic]
    ...
    vars = 'motor turb PID shutdown_fn'
    vals = 'motor_torque turbine_torque initial_motor_PID:output motor_torque_fn_shutdown'
  []
  ...
  #Turns on "reactor core". After 5000 s to establish coolant flow it ramps power from 0 to maximum value using power_fn
  [power_logic]
    type = ParsedFunctionControl
    function = 'if(t > 1000, power_fn, zero)'
    vars = 'power_fn zero'
    vals = 'power_fn 0'
  []

  #Applies the output from [power_logic] to [total_power] in [Components]
  [power_applied]
    type = SetComponentRealValueControl
...
[Controls]
  ...
  [PID_trip_status]
    ...
    enable_objects = 'AuxScalarKernels::set_PID_tripped'
    execute_on = 'TIMESTEP_END'
  []

  #Enalbes set_time_PID
  [time_PID]
    type = ConditionalFunctionEnableControl
    conditional_function = get_PID_trip_status_fn
    disable_objects = 'AuxScalarKernels::set_time_PID'
    execute_on = 'TIMESTEP_END'
  []
[]
...
[AuxScalarKernels]
  ...
  #Overwrites variable PID_trip_status to the value from set_PID_tripped_fn (changes 0 to 1)
  [set_PID_tripped]
    type = FunctionScalarAux
    function = set_PID_tripped_fn
